...
def test_read_root(client):
    ...
    This test verifies that the root endpoint correctly returns the HTML page.
    It checks:
    - Status code is 200 OK.
    - Content-Type header is 'text/html'.
    - The HTML body contains the 'Hello World' heading.

    Steps:
    1. Send a GET request to the '/' endpoint.
    2. Assert the status code and content type.
    3. Assert that the response text contains the expected HTML element.
    """
    ...
    assert 'text/html' in response.headers['content-type'], \
        f"Expected content-type 'text/html', got '{response.headers['content-type']}'"
    
    # Assert that the response body contains the '<h1>Hello World</h1>' markup
    assert 'Hello World' in response.text, \
        "Expected 'Hello World' in the HTML response"